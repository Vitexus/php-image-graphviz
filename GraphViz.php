 ...
 * PHP version 5
 *
 * Copyright (c) 2001-2007, Dr. Volker Gobbels <vmg@arachnion.de> and
 ...
 * @author    Dr. Volker Gobbels <vmg@arachnion.de>
...
class Image_GraphViz
{
     ...
     * @access public
     */
    function __construct($directed = true, $attributes = array(), $name = 'G', $strict = true, $returnError = false)
    {
        $this->System = new System();
        $this->PEAR = new PEAR;
        $this->Image_GraphViz($directed, $attributes, $name, $strict, $returnError);
    }
     ...
     * @access public
     */
    function Image_GraphViz($directed = true, $attributes = array(), $name = 'G', $strict = true, $returnError = false)
    {
    ...
    function fetch($format = 'svg', $command = null)
    {
        ...
        if (!$fp) {
            ...
            $error = $this->PEAR->raiseError('Could not read rendered file');
            return $error;
        }
    ...
    function renderDotFile($dotfile, $outputfile, $format = 'svg', $command = null)
    {
        if (!file_exists($dotfile)) {
            ...
            $error = $this->PEAR->raiseError('Could not find dot file');
            return $error;
        }
        ...
        $error = $this->PEAR->raiseError($command_orig . ' command failed: '
            . implode("\n", $msg));
        return $error;
    }
    ...
    function save($file = '')
    {
        ...
        if (empty($file)) {
            $file = $this->System->mktemp('graph_');
        }
        ...
        $error = $this->PEAR->raiseError('Could not save serialized graph instance');
        return $error;
    }
    ...
    function saveParsedGraph($file = '')
    {
        ...
        $error = $this->PEAR->raiseError('Could not save graph');
        return $error;
    }
